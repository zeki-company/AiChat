...
public class FixedTextureVideoView extends TextureView
    ...
    public void setFixedSize(int width, int height) {
        fixedHeight = height;
        fixedWidth = width;
        Timber.d(TAG, "setFixedSize,width=" + width + "height=" + height);
        requestLayout();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Timber.d(TAG, String.format("onMeasure, fixedWidth=%d,fixedHeight=%d", fixedWidth, fixedHeight));
    }
    ...
    private void transformVideo(int videoWidth, int videoHeight) {
        if (getResizedHeight() == 0 || getResizedWidth() == 0) {
            Timber.d(TAG, "transformVideo, getResizedHeight=" + getResizedHeight() + "," + "getResizedWidth=" + getResizedWidth());
            return;
        }
        float sx = (float) getResizedWidth() / (float) videoWidth;
        float sy = (float) getResizedHeight() / (float) videoHeight;
        Timber.d(TAG, "transformVideo, sx=" + sx);
        Timber.d(TAG, "transformVideo, sy=" + sy);
        ...
        Timber.d(TAG, "transformVideo, maxScale=" + maxScale);

        setTransform(matrix);
        postInvalidate();
        Timber.d(TAG, "transformVideo, videoWidth=" + videoWidth + "," + "videoHeight=" + videoHeight);
    }
            ...
            new MediaPlayer.OnVideoSizeChangedListener() {
                public void onVideoSizeChanged(MediaPlayer mp, int width, int height) {
                    ...
                    if (mVideoWidth != 0 && mVideoHeight != 0) {
                        ...
                        Timber.d(TAG, String.format("OnVideoSizeChangedListener, mVideoWidth=%d,mVideoHeight=%d", mVideoWidth, mVideoHeight));
                    }
                }
            };
    ...
    private OnErrorListener mErrorListener =
            new OnErrorListener() {
                public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
                    Timber.d(TAG, "Error: " + framework_err + "," + impl_err);
                    mCurrentState = STATE_ERROR;
                    mTargetState = STATE_ERROR;
                    if (mMediaController != null) {
                        mMediaController.hide();
